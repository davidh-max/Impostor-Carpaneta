...
const CrestLogo = ({ className = "h-8 w-8" }: { className?: string }) => (
  <div className={`${className} bg-white rounded-full border-2 border-[#0B0B0B] overflow-hidden flex items-center justify-center`}>
    <img 
...
const Header = ({ onHomeClick, onExitClick, showExit }: { onHomeClick?: () => void, onExitClick?: () => void, showExit?: boolean }) => (
  <header className="flex flex-row items-center justify-between px-6 py-4 sticky top-0 bg-white/95 backdrop-blur-sm z-30 border-b-2 border-[#0B0B0B]/10">
...
const Button = ({ children, onClick, variant = 'primary', className = '', disabled = false, fullWidth = false }: any) => {
  const variants: any = {
    primary: 'bg-[#FE70C8] text-white border-2 border-[#0B0B0B] sticker-shadow-sm active:translate-x-[1px] active:translate-y-[1px] active:shadow-none',
    secondary: 'bg-white text-[#0B0B0B] border-2 border-[#0B0B0B] sticker-shadow-sm active:translate-x-[1px] active:translate-y-[1px] active:shadow-none',
    danger: 'bg-red-500 text-white border-2 border-[#0B0B0B] sticker-shadow-sm active:translate-x-[1px] active:translate-y-[1px] active:shadow-none',
    ghost: 'bg-transparent text-[#5F5E5E] hover:text-[#FE70C8]',
    neon: 'bg-[#FE70C8] text-white font-black uppercase tracking-wider border-2 border-[#0B0B0B] sticker-shadow active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all'
  };
  return (
    ...
    try {
      ...
      if (category === 'Personalizada') {
        ...
        if (impostorHintEnabled) {
          // Obtener pista dinámica de Gemini con fallback
                ...
                <div className="mt-8 pt-6 border-t border-white/20 w-full">
                  <Button variant="secondary" fullWidth onClick={() => {
                    if (gameState.revealIndex < gameState.players.length - 1) {
                      ...
                      setGameState(prev => ({ ...prev, phase: 'DISCUSSION', isTimerRunning: true }));
                    }
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 pb-24">
        ...
        <div className="mt-8 space-y-3">
          ...
          {gameState.players.map(p => (
            <div key={p.id} className="bg-white p-4 rounded-2xl border-2 border-[#0B0B0B]/10 flex justify-between items-center"><span className="font-bold text-[#0B0B0B] uppercase text-sm">{p.name}</span><div className="w-2 h-2 rounded-full bg-[#FE70C8] opacity-30"></div></div>
          ))}
        </div>
      </div>
      <div className="p-6 bg-white border-t-2 border-[#0B0B0B]/5 sticky bottom-0"><Button variant="neon" fullWidth onClick={() => setGameState(prev => ({ ...prev, phase: 'VOTE' }))}><Vote size={22} /><span>FINALIZAR DEBATE</span></Button></div>
    </div>
  );
